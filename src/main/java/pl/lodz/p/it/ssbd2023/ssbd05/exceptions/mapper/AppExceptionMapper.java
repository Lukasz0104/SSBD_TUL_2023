...
public class AppExceptionMapper implements ExceptionMapper<AppBaseException> {
    ...
    public Response toResponse(AppBaseException e) {
        ...
        Map<Class<?>, Response.Status> map = Map.of(AppConflictException.class, Response.Status.CONFLICT, AppBadRequestException.class, Response.Status.BAD_REQUEST, AppUnauthorizedException.class, Response.Status.UNAUTHORIZED);
        return Response.status(map.getOrDefault(e.getClass(), Response.Status.INTERNAL_SERVER_ERROR)).build();
    }
}
